...
# Set the prompt ...
source $HOME/.bashprompt/functions/bashprompt.function
callbashprompt gc
...
# revert any outstanding changed in current dir and below
svnrevert()
{
  svn st "${@}" | awk '{ print $2 }' | xargs svn revert 
}

export PATH=$PATH:/home/ryan/bin/:/opt/wireshark/bin/

export TODOTXT_SORT_COMMAND='env LC_COLLATE=C sort -k 2,2 -k 1,1n'

alias t='todo.sh -d /home/ryan/bin/todo.cfg'

git-svn-outgoing() {
  need_stash=$(git status --por | cut -c 1-2 | grep M | cat)
  if [ "$need_stash" ]; then
    git stash > /dev/null
  fi
